        ...
        <form @submit.prevent="submit">
          <div class="dialog-body">
            ...
            <div v-if="advanced" class="advanced-fields">
              ...
              <label><span>LÍMITE DE VELOCIDAD</span><select v-model.number="speedLimitBytes" name="speed-limit"><option v-for="limit in speedLimits" :key="limit.value" :value="limit.value">{{ limit.label }}</option></select><small>Se aplica al total de todos los segmentos.</small></label>
...
.dialog-card{width:min(680px,100%)}.quick-grid{grid-template-columns:minmax(0,1fr) 130px 110px}.advanced-fields{grid-template-columns:1fr 1fr}.advanced-fields>.warnings{grid-column:1/-1;margin-top:0}.destination-field>span button{float:right;padding:0;border:0;background:transparent;color:var(--accent);font:8px var(--mono);cursor:pointer}.destination-field>small{display:block;margin-top:5px;color:#5c6761;font:8px var(--mono)}
