...
import { IngestError, normalizeHeaders, parseCookies, parseHeaderLines, parseRequest } from "@/domain/ingest";
...
const additionalHeaderWarnings = computed(() => {
  try {
    return normalizeHeaders(parseHeaderLines(extraHeaders.value)).warnings;
  } catch {
    return [];
  }
});
...
watch(() => props.open, (open) => {
  ...
  speedLimitBytes.value = props.settings.defaultSpeedLimitBytes;
...
function submit(): void {
  ...
  try {
    ...
    const additional = normalizeHeaders(parseHeaderLines(extraHeaders.value));
    const headers = [...request.source.headers, ...additional.headers];
    const cookies = [...request.source.cookies, ...additional.cookies, ...parseCookies(extraCookies.value)];
    emit("submit", {
      source: {
        ...request.source,
        headers,
      ...
      fileNameCustomized: fileNameCustomized.value && fileName.value.trim().length > 0,
      category: category.value,
      categoryCustomized: categoryCustomized.value,
      destination: destination.value,
      destinationCustomized: destinationCustomized.value,
      threads: threads.value,
      speedLimitBytes: speedLimitBytes.value,
        ...
        <form @submit.prevent="submit">
          <div class="dialog-body">
            ...
            <div class="quick-grid">
              ...
              <label><span>MÁX. SEGMENTOS</span><select v-model.number="threads" name="threads"><option v-for="value in [1,2,4,6,8,12,16,24,32]" :key="value" :value="value">Hasta {{ value }}</option></select></label>
            </div>
            ...
            <div v-if="advanced" class="advanced-fields">
              ...
              <div class="speed-limit-field"><span>LÍMITE DE VELOCIDAD</span><SpeedLimitInput v-model="speedLimitBytes" name="speed-limit" /><small>Se aplica al total de todos los segmentos.</small></div>
              ...
              <div v-if="additionalHeaderWarnings.length" class="warnings"><p v-for="warning in additionalHeaderWarnings" :key="warning">{{ warning }}</p></div>
...
.dialog-card{width:min(680px,100%)}.quick-grid{grid-template-columns:minmax(0,1fr) 130px 110px}.advanced-fields{grid-template-columns:1fr 1fr}.advanced-fields>.warnings{grid-column:1/-1;margin-top:0}.speed-limit-field{grid-column:1/-1}.speed-limit-field>span{display:block;margin-bottom:7px;color:#7b8580;font:8px var(--mono);letter-spacing:.11em}.speed-limit-field>small{display:block;margin-top:5px;color:#5c6761;font:8px var(--mono)}.destination-field>span button{float:right;padding:0;border:0;background:transparent;color:var(--accent);font:8px var(--mono);cursor:pointer}.destination-field>small{display:block;margin-top:5px;color:#5c6761;font:8px var(--mono)}
