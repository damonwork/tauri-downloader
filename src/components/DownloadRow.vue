...
<template>
  <article class="download-row" :class="{ selected }" tabindex="0" @click="emit('select')" @keydown.enter="emit('select')">
    ...
    <div class="transfer-cell">
      ...
      <small>{{ item.state.kind === 'downloading' ? `${formatEta(total, item.transfer.downloadedBytes, speed)} restantes` : `${item.threads} hilos` }}</small>
    </div>
