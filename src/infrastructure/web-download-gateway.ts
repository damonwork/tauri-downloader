...
import type { CreateDownloadInput, DownloadAction, DownloadItem, DownloadSource, SegmentState, TransferTelemetry } from "@/domain/download";
import { categoryForFile, EMPTY_TRANSFER_TELEMETRY, segmentSize } from "@/domain/download";
  ...
  private schedule(): void {
    ...
    this.state.downloads
      ...
      .forEach((item) => {
        ...
        if (item.transfer.size.kind === "known" && item.telemetry.segments.length === 0) {
...
function emptyTelemetry(): TransferTelemetry {
  return structuredClone(EMPTY_TRANSFER_TELEMETRY);
}

function normalizeTelemetry(telemetry: TransferTelemetry | undefined): TransferTelemetry {
...
function legacyTelemetry(item: DownloadItem): TransferTelemetry {
